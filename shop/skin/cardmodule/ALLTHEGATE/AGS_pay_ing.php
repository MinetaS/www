...
$VIRTUAL_CENTERCD = trim($_POST["VIRTUAL_CENTERCD"]);   //VIRTUAL_CENTERCD(무통장입금) - 은행코드 가상계좌추가
  
$VIRTUAL_DEPODT = trim($_POST["VIRTUAL_DEPODT"]);		//VIRTUAL_DEPODT(무통장입금) - 입금예정일 가상계좌추가
 
$ZuminCode = trim($_POST["ZuminCode"]);					//ZuminCode(무통장입금) - 주민번호 가상계좌추가

$MallUrl = trim($_POST["MallUrl"]);						//MallUrl(무통장입금) - 상점 도메인 가상계좌추가

$MallPage = trim($_POST["MallPage"]);					//MallPage(무통장입금) - 상점 입/출금 통보 페이지 가상계좌추가

$VIRTUAL_NO = trim($_POST["VIRTUAL_NO"]);				//VIRTUAL_NO(무통장입금) - 가상계좌번호 가상계좌추가
...
<input type=hidden name=rVirNo value="<?=$rVirNo?>"><!-- 가상계좌추가 -->
