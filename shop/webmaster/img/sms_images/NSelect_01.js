/*
 * 이미지 경로를 static 서버에 있는것을 쓰기위해 수정함 kd579@nhncorp.com 
 *	modify 2003.11.05
 * 셀렉트박스 선택변경시 이벤트 발생 부분 추가 (NSelect1) aggom@nhncorp.com 
 *  modify 2004. 6. 8
 */
...
var SL_Text = "text-indent:3px;padding-top:2px;";
var SL_IPrefix = "http://image.mobile.naver.com/img/";
var SL_IPrefix2 = "http://static.naver.com/common/bu/";
var SL_AImage = "bu_tu04.gif";
var SL_BImage = "sp.gif";
var SL_SBLen = 10;
var SL_SBWidth = 15;
var SL_ScrollBar = ""
...
// parameter NSelect
function NSelect1( HTMLSrc, KIN, BG, BM, BD, AI, strOnBeforeChangeEventHandlerName )
{
    if ( UserAgent.indexOf( "MSIE" ) < 0 || AppVersion < 5 ) {
	document.write( HTMLSrc );
	return;
	...
	var SE = setEnv( HTMLSrc, BG, BM, BD, AI );
	var SListObj = new setNSelect1( HTMLSrc, KIN, SE, strOnBeforeChangeEventHandlerName );
	SListObj.append();
...
function ListMouseDownHandler( f )
{
	...
	if ( i == f.idx ) {
	    ...
	    SI_Val = this.Items.options[i].value;
	}
    }
...
//added by aggom (2004.6.8)
function setNSelect1( pSrc, pKIN, pSE, strOnBeforeChangeEventHandlerName )
{
    this.entry = SList.length;
    this.lower = null;
    this.src = pSrc;
...
function createTitle()
{
	...
	+"<img style='position:absolute;top:0px;left:0;width:"+this.env.Width+";height:"+this.env.Height+";'"
...
function openPaymentPop(url)
{
	var left_p = (screen.width - 420)/2;
	var top_p = (screen.height - 480)/2;
	window.open(url,"noticewin","scrollbars=no,resizable=no,top=40,left=40,width=420,height=480,left=" + left_p + ",top=" + top_p);
}

function selPriceInfo(telecom) {
	
   	if  ( telecom == 'K' )  {
		document.all.telecomk.style.display  = "inline";
		document.all.tabK.style.display  = "inline";
	} else {
		document.all.telecomk.style.display  = "none";
		document.all.tabK.style.display  = "none";
	}

   	if  ( telecom == 'S' )  {
		document.all.telecoms.style.display  = "inline";
		document.all.tabS.style.display  = "inline";
	} else {
		document.all.telecoms.style.display  = "none";
		document.all.tabS.style.display  = "none";
	}
	
	if  ( telecom == 'L' )  {
		document.all.telecoml.style.display  = "inline";
		document.all.tabL.style.display  = "inline";
	} else {
		document.all.telecoml.style.display  = "none";
		document.all.tabL.style.display  = "none";
	}
}

function OpenWindow(url, formname, product_make, product_title, target,width,height,scroll,toolbar,resiz,stat,name,temp) 
{
	...
	product_make = product_make?product_make:"";
	product_title = product_title?product_title:"";

	if(temp==2) surl = url;
	else surl = url + "?formname=" + formname + "&product_make=" + product_make + "&product_title=" + product_title + "&temp=" + temp;
    window.open(surl, name, "width="+ width + ", height=" + height + ", toolbar=" + toolbar + ", resizable=" + resiz + ", scrollbars=" + scroll + ", status=" + stat + ", top=" + window_top + ", left=" + window_left);
}
