...
<table class='toolbar' cellspacing='0' cellpadding='0' width='90%' align='center'>
	...
	<tr height='30'>
		...
		<td background='".$this->skinpath."images/sidebar/titlebg.png' align='center' valign='middle'><b>".$bhlang['title:folderdz']."</b></td>
...
foreach ($this->content1 as $username=>$emailarray) {
	...
	foreach ($emailarray as $email=>$codearray) {
		foreach ($codearray as $filecode=>$filearray) {
			...
			$str .= "<tr><td> &nbsp; &nbsp; </td><td>$email</td><td>$filepath</td><td>".bh_humanise_time($expiresin)."</td><td><a href='$filelink'>".$bhlang['button:link']."</a> &nbsp; <a href='index.php?page=folderdz&deletelink=$filecode'>".$bhlang['button:delete']."</a></td></tr>";
		}
	}
