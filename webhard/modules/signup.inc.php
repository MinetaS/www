		...
		if ((!empty($usernamerows)) || (!empty($regusernamerows)) || ($username == "guest") || ($username == "admin") || ($username == "administrator") || ($username == "all")) {
			...
			# Or the emails
				...
				*/
				...
				// 바로 유저 등록 하기 15.06.29
				# Add a user row
				insert_bhdb("users", array("username"=>$username, "password"=>md5($signup['pass1']), "type"=>"normal", "homedir"=>"/".$username,"quota"=>"314572800"));
				# Add that extra info we got
				insert_bhdb("userinfo", array("username"=>$username, "itemname"=>"fullname", "itemcontent"=>$signup['fullname']));
				insert_bhdb("userinfo", array("username"=>$username, "itemname"=>"email", "itemcontent"=>$signup['email']));
				# Delete the reg row
