 ...
 *   $Id: filesystem.inc.php,v 1.3 2005/03/20 17:40:04 andrewgodwin Exp $
 *
 */
...
class bhfile {
	...
	# Chooses which method to do, based on config
	...
	function uploaded_file($tmppath) {
		...
		if ($bhconfig['webserving'] == 1) {
			chmod($this->absfilepath, 0755);
		}

		# Update filesize
		update_bhdb("metadata", array("metacontent"=>filesize($this->absfilepath)), array("filepath"=>$this->filepath, "metaname"=>"filesize"));
	}
	...
	function set_publicrights($status) {
	...
	function set_userrights($username, $status) {
...
function bh_fpclean($filepath) {
	...
	$filepath = str_replace("/..", "", $filepath);		# Get rid of any nasty directory ups.
...
function bh_mkdir($filepath) {
	...
	return mkdir($bhconfig['fileroot'].$filepath, 0707);
}
